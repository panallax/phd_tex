...
Modifying the position of the nodes to homogenise the length of these paths would ensure that the stresses are also distributed evenly throughout the structure.
This would prevent certain areas from easily exceeding critical stresses. 
The problem with this proposal is that it requires modifying the position of the nodes and edges, causing the structure to lose its self-supporting condition. 
Therefore, once the geometry of the structure is defined, the only remaining variable that can contribute rigidity to the structure is the section of the bars. 
...
And, also, increasing the moment of inertia increases the critical Euler buckling load of the bar. 
...
To do this, it is proposed to find the combination of sections for all the bars that provides the highest compressive strength and the lowest possible mass. 
...
Although matrix analysis of structures does not offer real results, due to the simplifications it assumes, it allows the stress at the nodes to be calculated very quickly. 
Therefore, its inclusion in an iterative optimisation process is feasible. 
Another alternative would be to perform the calculations using finite elements analysis (FEA) simulations, but this would greatly increase the computing time, and the results might not be so different from those obtained using matrix analysis.   
...
Once the global stiffness matrix of the structure has been obtained, the nodal force vector must be defined in order to solve \cref{eq:stiff} and thus obtain the nodal displacement vector.
...
\subsection{Optimisation algorithm definition}
...
In order to reduce the non-linearity of the problem, the possible values of the diameters of each of the bars are limited according to their mechanical slenderness and their critical buckling load. 
This prevents the bars from buckling and entering a non-linear elastic regime. 
In order to apply Euler's buckling theory, the bars must be very slender. 
...
If $\eta >1$ means that the bar has collapsed while $\eta \leq 1$ means that the bar remains in the elastic regime and still functional.
...
This is done with the intention of introducing diversity into the population and avoiding possible stagnation at local minimum. 
...
As an example of how the optimisation algorithm works, the distribution of the diameters of the first layer of tetrahedrons in a structure generated from a cylinder with a diameter of 80 \textit{mm} has been optimised. 
...
\cref{fig:optim} \textcolor{blue}{B} shows the evolution of the structure's weight, the maximum utilisation factor among the structure and average diameter. Conventional steel has been used as the structural material, with the following mechanical properties: $E$ = 200 $GPa$, $G$= 77 $GPa$, $\nu$ = 0.3, $\rho$ = 7.85$\times10^{-6}\;kg/mm^3$ and $\sigma_Y$ = 235 $MPa$.

\begin{figure}[!htbp]
    \centering
    \begin{subfigure}[b]{0.5\textwidth}
        \\includegraphics[width = \textwidth]{imgs/G_4.svg.pdf}
    ...
    \begin{subfigure}[b]{0.8\textwidth}
        ...
        \caption{Evolution of the different variables to be optimised during optimisation}
     \end{subfigure}
     \caption{Result of the optimisation of the first layer of tetrahedrons generated from a cylinder with a diameter of 80 \textit{mm}.}
    \label{fig:optim}
\end{figure}
