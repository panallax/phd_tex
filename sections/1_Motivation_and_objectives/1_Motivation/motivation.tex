...
Three-dimensional (3D) bioprinting involves the use of techniques similar to 3D printing to combine cells, growth factors, biological inks and biomaterials to manufacture functional structures that were traditionally used for tissue engineering applications. 
One such application is 3D bone tissue printing, which aims to design structures, or bone scaffolds, that can contain living bone tissue and partially or totally replace a bone that needs to be replaced.

The design of these scaffolds is very complex, as they must be able to withstand all the loads that the bone supports and, in addition, they must allow the regeneration of the internal vascular tissue of the bone\cite{life12060903}. 
Apart from the design of these structures, bone bioprinting also poses other challenges, such as generating bio-inks that keep bone cells alive and using 3D printing methods that are compatible with the use of these bio-inks\cite{jfb13040214}.

This Thesis aims to investigate whether it is possible to design self-sustaining stochastic porous structures that can be used as bone scaffolds and whether their application beyond bioprinting is possible.
...
The loss of large amounts of bone tissue, caused by trauma, tumour resection or degenerative diseases, poses a significant clinical challenge, as bone is unable to regenerate in critical defects.
In this situation, prosthetic solutions are required that restore structural function and promote biological integration with the surrounding tissue.
...
Firstly, their high rigidity compared to that of bone causes \textit{stress shielding}, i.e. an abnormal redistribution of loads that leads to the resorption of adjacent bone and a decrease in long-term stability \cite{Huiskes1992, Sumner2015}.
Secondly, as they are non-resorbable materials, metal prostheses do not integrate dynamically with the body, but remain as foreign bodies that sometimes require revision surgery after prolonged periods \cite{Pioletti2010}.
...
Given these limitations, bone scaffolds represent a very promising alternative. 
Their main advantage is that they can be designed with a porous, interconnected microstructure that mimics the hierarchical architecture of bone, which promotes cell migration, nutrient transport and vascular growth.
...
\begin{figure}[!htbp]
    ...
    \caption{Pbroks13. (2012). Long bone diagram. \href{https://commons.wikimedia.org/wiki/File:Bone_cross-section.svg}{Wikimedia Commons}. CC BY-SA 4.0.}
    \label{fig:bone}
\end{figure}
...
Therefore, the preparation of bone scaffolds with adequate pore size and interconnectivity is an important issue in bone tissue engineering. 
...
\begin{figure}[!htbp]
    ...
    \caption{Different types of lattice unit cells (A) Strut-based or geometrical lattice (B,C) TPMS-based unit cells. Image extracted with Creative Commons CC-BY-NC-ND 4.0 permissions from Benedetti et al. Figure 6 \cite{BENEDETTI2021100606}}
    \label{fig:regular}
\end{figure}

At the opposite end of the spectrum are random or stochastic designs, which are designed to more closely mimic the irregular nature of bone. 
...
\begin{figure}[!htbp]
    ...
    \caption{Example of stochastic design for bone scaffold.}
    \label{fig:random}
\end{figure}
...
Between these two approaches lies an opportunity: the development of self-supporting stochastic structures. 
This type of design seeks to reproduce the spatial heterogeneity and biological advantages of random scaffolds, but with a geometric restriction that guarantees their stability during printing without the need for supports.
...
At the same time, adequate mechanical properties are maintained, and a favourable environment for bone regeneration is facilitated. 
These characteristics make self-supporting scaffolds an ideal candidate for advancing towards customised prostheses that combine industrial viability with maximum biological potential.
...
Overall, it can be said that conventional 3D printing technologies are optimal for manufacturing structural scaffolds but are not compatible with the presence of living cells during printing. 
