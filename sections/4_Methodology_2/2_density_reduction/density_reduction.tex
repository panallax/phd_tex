...
An example of this can be seen in the left image of \textcolor{blue}{Figure} \ref{fig:shell_points_2}. 
On the other hand, if the vertexes are removed based on some criteria, those undesired vertices can be removed to get a more uniform distribution.
...
In the \textcolor{blue}{Figure} \ref{fig:merge_points}, an example of this process is illustrated.
...
There may be some situations where one vertex is too close to two vertexes, or even more.
In such cases, the set of vertexes that link all the vertices in the set are merged together at the midpoint of the set.
In the \textcolor{blue}{Figure} \ref{fig:merge_segments} is shown an example of the output of this process.
...
It should be noted that the set of points represented corresponds only to the vertices created, and that the shell points have not been joined at this point in order to avoid movement of the shell points. 
These points must be fixed as they belong to the shell.
Initially there were 126 vertexes, and after the process only 92 remain.
As can be seen in the figure, the result of this process is a triangulation where the small triangles have disappeared.
There could be situations where the process of merging points generate new points that are still too close to other points.
To avoid that, the process continues until there is no more short segments to remove.

\vskip 0.3cm
\begin{figure}[!htbp]
\centering
\input{imgs/remove_segments.tex}
\caption{Procedure to remove short segments. The dotted lines represent edges at the back, while the dashed lines represent the edges of the triangulation. The image A shows the initial state where two vertices from the two neighbouring simplices are generated too close. Image B shows the final state from the merging process, where this short segment is removed.}
\label{fig:merge_points}
\end{figure}

\begin{figure}[H]
    \\includegraphics[width=1\textwidth]{imgs/remove_segments.svg.pdf}
    \centering
...
The \textcolor{blue}{Figure} \ref{fig:3d_apexes} shows a triangulation of a set of points whit small decreasing triangle area gradient from left to right.
...
\begin{figure}[h!]
    \\includegraphics[width=0.8\textwidth]{imgs/3d_apexes.svg.pdf}
    \centering
    \caption{Representation of the triangulation of a set of points with a decreasing triangle area gradient from the left to the right. From those triangles and the apexes obtained from them, a surface is generated showing that the height of each tetrahedron is proportional to the area of the triangle.}
\label{fig:3d_apexes}
\end{figure}
...
The area of the triangles is computed as half of the norm of the cross product of the vectors that form the triangle (\textcolor{blue}{Equation} \ref{eq:area}).
...
In the \textcolor{blue}{Figure} \ref{fig:small_area}, a detail of the output of this routine is shown.
...
The result of this routine in the triangulation of the set of points is shown in the \textcolor{blue}{Figure} \ref{fig:euler} \textcolor{blue}{B} can be observed in the \textcolor{blue}{Figure} \ref{fig:small_area_2}.
In the figure is shown the initial triangulation is shown in dotted magenta lines, and the final tessellation in green lines. 
...
\begin{figure}[htbp!]
    \\includegraphics[width=0.4\textwidth]{imgs/small_area.svg.pdf}
    \centering
...
\begin{figure}[!hbtp]
    \\includegraphics[width=0.9\textwidth]{imgs/small_area_2.svg.pdf}
    \centering
    \caption{Example of the result of the process of increasing the area of the small triangles. The initial triangulation is represented in dotted magenta lines, and the final tessellation in green lines. The red points represent the new positions of the vertices that have been moved to the centroid of the set of points, while the points marked caret down represent the initial distribution of the vertices.}
...
In the \textcolor{blue}{Figure} \ref{fig:optimized} is shown the input and output results of the process of removing short segments and increasing the area of the small triangles.
The initial triangulation in the example is made up of 153 vertices, while only 118 remain after the process.
That is translated into a reduction from 277 simplices to 211 and a smoother and more uniform tessellation, although some small triangles still remain.


\begin{figure}[!hbtp]
    \\includegraphics[width=1\textwidth]{imgs/optimized.svg.pdf}
    \centering
...
\newpage
A brief analysis of the variation of the heights and areas from the triangles in the \textcolor{blue}{Figure} \ref{fig:optimized} is shown in the \textcolor{blue}{Figure} \ref{fig:distribution}.
...
\begin{figure}[!hbtp]
    \\includegraphics[width=0.8\textwidth]{imgs/distributions.svg.pdf}
    \centering
    \caption{Distribution of the heights and areas of the tetrahedrons in the initial triangulation and after the process of removing short segments and increasing the area of the small triangles. The blue bars represent the distribution of the heights of the tetrahedra in the initial triangulation, while the orange bars represent the distribution of the heights of the tetrahedra after the processing. Also, the mean values of each distribution are represented by the dashed lines, cyan for the initial distribution and red for the final distribution.}
...
In order to answer this question, it is necessary to look at the \textcolor{blue}{Equation} \ref{eq:euler_3}.
From this equation can be noticed that the number of simplices generated is almost twice the number of vertexes.
...
In the example shown, the number of vertexes has been reduced by almost a quarter, which is not enough to keep the vertex density constant.
It is questionable whether it is only in the example used that the desired reduction is not achieved, but that perhaps in other situations it is. 
...
Other than this, in the previous section, it was explained that in order to obtain the position of each vertex, \textcolor{blue}{Problem} \ref{eq:opt_func_sum} was solved for each of the triangles. 
This problem is formulated in such a way that the solution obtained complies with the angularity constraints. 
Then, one could realise that it does not make sense to base the solution of the point density control on the movement of the generated points since the simple fact of moving these points may be causing the angularity conditions are no longer be fulfilled. 
