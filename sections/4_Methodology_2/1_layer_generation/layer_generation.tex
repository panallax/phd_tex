...
\begin{document}
From the attempts commented on in \cref{sec:meth_1} arises the following question: 
...
An example of the tessellation of a set of points of the base of a cylinder with a diameter of 3 \textit{mm} is shown in \cref{fig:tessellation}.

\begin{figure}[!htbp]
    \includegraphics[width=1\textwidth]{imgs/tessellation.eps}
    \centering
    \caption{Example of tessellation of a set of points of the base of a cylinder with a diameter of 3 \textit{mm}}
...
To accomplish this, given N points representing the points of the base of the pyramid, the objective is to find the point that minimises the inner product between the vector formed by each base point and this point, and the vector normal to the horizontal plane, $\vec{n_z} = (0,0,-1)$.
...
Since the inner product between two vectors is equal to the product of the magnitudes of the vectors and the cosine of the angle between them (\cref{eq:inner_product}), the angle between the vectors can be calculated as the arccosine of the inner product divided by the product of the magnitudes of the vectors (\cref{eq: angle}).
So, the function to optimize, $g(\vec{a}, \vec{b})$ is the arccosine of the inner product between the vector formed by each base point and the point to find, and the vector normal to the horizontal plane, minus 45 degrees (\cref{eq:opt_func}).
...
This function, $g(\vec{a}, \vec{b})$ (\cref{eq:opt_func}), must be evaluated for each of the N points of the base. 
...
Then, the objective function to minimise would be the sum of the result of the function for each of the points of the base (\cref{eq:opt_func_sum}).
...
Leaving $f(\vec{x})$ and $g(\vec{x})$ as functions of the coordinates of the point to be found, $\vec{x} = (x, y, z)$.
...
Then, the optimisation problem to be solved is formulated in \cref{opt:opt_problem}.
...
\begin{mini}
    {x, y, z}{\sum_{1}^{N} \arccos(\frac{z_n - z}{\lVert \vec{(x_n - x, y_n - y, z_n - z)}\rVert \cdot \lVert \vec{n_z} \rVert}) - 45}{}{}
...
An example of the result of the optimisation process is shown in \cref{fig:optimization}.
...
\begin{figure}[!htbp]   
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/opt_func.png}
...
An example of the result of this process is shown in \cref{fig:pyramids}.
...
\begin{figure}[!htbp]
    ...
    \caption{Example of the result of the process of finding the apex of the pyramids for each of the simplices of the tessellation of the base of a cylinder with a diameter of 3 \textit{mm}.}
    \label{fig:pyramids}
\end{figure}
...
But, as it can be seen in the \cref{fig:optimization}, the apexes created do not lie on the surface of the cylinder, but they are inside it.
...
To calculate the width of this range, the characteristic distance of the shell points has been calculated by calculating the cumulative sum of the distance of each point to its nearest neighbour and dividing it by the number of points (\cref{eq:char_dist}), 
...
An example of how the width of the range affects the number of points extracted is shown in \cref{fig:shell_points}.
...
In the \cref{fig:shell_points_3}, an example of this situation is shown. 
The section from the middle image of the \cref{fig:shell_points} is represented from a three-dimensional view.
...
An example of this narrow triangle can be seen in the right image of the \cref{fig:shell_points_2}.

\begin{figure}[!htbp]
    \includegraphics[width=0.7\textwidth]{imgs/shell_points_3.png}
    \centering
    \caption{Representation of the shell points extracted by taking the average height of the points. Orange points represent the hull points of the points from the \cref{fig:shell_points}, and the blue points represent the points extracted from the shell.}
...
In the top right of the right image in \cref{fig:shell_points_2}, and in the bottom, an example of this situation is shown.
...
\end{figure}
\footnotetext[6]{This image was created from the central image in \cref{fig:shell_points} by randomly removing the 80\% of the apexes.}
...
In the \cref{fig:shell_points_4}, an example of this change is shown. 
...
\begin{equation}
    ...
    \label{eq:edges}
\end{equation}
If \cref{eq:edges} is substituted into \cref{eq:euler}, the following is obtained:
\begin{equation}
    F = 2(V - 2)
    \label{eq:euler_2}
\end{equation}
...
Then, \cref{eq:edges} can be rewritten as:
...
If the \cref{eq:edges_2} is substituted into \cref{eq:euler}, the following is obtained:
...
This can be seen in \cref{fig:euler} where the number of simplices and points is shown for the first four layers. 
