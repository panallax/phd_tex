...
In this context, volume is understood as the geometry used to generate the structure inside. 
It must be one of the input parameters used to begin development in the form of an STL file.
...
The easiest way to achieve this is by generating random points inside the bounding box of the volume and then removing the points that are not inside the volume, as explained above.
...
Clearly, this is illogical in the macroscopic world in which we live.
...
This algorithm ensures that the points are uniformly and evenly distributed, ensuring that two points are not located at a minimum desired distance. 
...
In the \cref{fig:poisson}, a comparison between the uniform and Poisson-disk sampling is shown.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width =\textwidth]{imgs/poisson.pdf}
    \caption{Comparison between uniform (left) and Poisson-disk (right) samplings of 500 points in a 1 \textit{mm} side square. A minimum distance of 0.05 \textit{mm} was used for the Poisson-disk sampling.}
    \label{fig:poisson}
\end{figure}

The difference between the two distributions is quite evident. 
While the uniform distribution has clusters of points and bigger empty areas, the Poisson-disk distribution achieves a more evenly distributed result. 
...
An example of the result of applying this logic to a 30x20 \textit{mm} cylindrical volume can be seen in \cref{fig:random_points}.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width =0.5\textwidth]{imgs/random_points.pdf}
...
\end{equation}
Initially, every pair of points forming an angle exceeding 45$^{\circ}$ was connected. Yet, this approach led to too many connections with excessive lengths. 
Consequently, it was decided that it was also necessary to regulate the behaviour of unions.
...
This value was obtained from a previous quantitative analysis to obtain a value that would ensure at least eight neighbours were reached. 
However, this value will depend on the number of points present in the volume.
...
\cref{fig:first_test_1} shows an example of a point cloud that has been joined using this method. 
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width =1.1\textwidth]{imgs/first_test_1.pdf}
...
Therefore, crossing edges makes structures unprintable in a non-planar way.

A significant amount of effort was put into fixing these errors, but due to the randomness present in the point generation process, it was not possible to guarantee that this method would always work.  
Therefore, it was decided to discard it in favour of finding a method to control randomness to ensure the presence of potential connections.
